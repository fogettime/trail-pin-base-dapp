...
const source = await readFile(resolve(process.cwd(), "contracts/TrailPin.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "TrailPin.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["TrailPin.sol"]?.TrailPin;
...
console.log(`Deploying TrailPin from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_TRAIL_PIN_CONTRACT_ADDRESS to this contract address in Vercel.");
